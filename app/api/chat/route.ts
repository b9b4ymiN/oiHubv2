  ...
  try {
    ...
    const processedMessages = messages.map((msg: any) => {
      ...
      if (msg.chart_context && msg.role === 'user') {
        ...
        const contextText = formatChartContext(msg.chart_context)

        // Prepend context to the user's message content
        const enhancedContent = `${contextText}\n\n---\n\nUser Question: ${msg.content}`

        console.log('[Chat API] Enhanced message with chart context:', msg.chart_context.type)
