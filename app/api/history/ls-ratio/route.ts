// app/api/history/ls-ratio/route.ts
//
// Historical Long/Short Ratio data endpoint.

import { NextRequest, NextResponse } from 'next/server'
import type { LongShortRatioRow } from '@/lib/db/schema'

export const dynamic = 'force-dynamic'

interface LSRatioResponse {
  success: true
  data: LongShortRatioRow[]
  meta: {
    symbol: string
    interval: string
    count: number
...
export async function GET(request: NextRequest): Promise<NextResponse<LSRatioResponse | ErrorResponse>> {
  ...
  try {
    ...
    // Check cache
    const cacheKey = getCacheKey('ls-ratio', {
      symbol,
      interval,
      start: String(start),
      end: String(end),
    ...
    const cached = getCached<LongShortRatioRow[]>(cacheKey)
    if (cached) {
    ...
    // Cache the result
    setCache(cacheKey, rows as LongShortRatioRow[], queryTimeMs)

    const response = NextResponse.json({
      success: true,
      data: rows as LongShortRatioRow[],
      meta: {
        symbol,
        interval,
        count: rows.length,
    ...
    logger.error({ error }, 'LS ratio history query failed')
