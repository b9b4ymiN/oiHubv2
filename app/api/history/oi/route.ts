// app/api/history/oi/route.ts
//
// Historical Open Interest data endpoint.

import { NextRequest, NextResponse } from 'next/server'
import type { OIRow } from '@/lib/db/schema'
...
export async function GET(request: NextRequest): Promise<NextResponse<OIResponse | ErrorResponse>> {
  ...
  try {
    ...
    const cached = getCached<OIRow[]>(cacheKey)
    if (cached) {
    ...
    // Cache the result
    setCache(cacheKey, rows as OIRow[], queryTimeMs)
    ...
    logger.error({ error }, 'OI history query failed')
