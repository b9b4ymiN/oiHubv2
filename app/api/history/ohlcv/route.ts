// app/api/history/ohlcv/route.ts
//
// Historical OHLCV data endpoint.

import { NextRequest, NextResponse } from 'next/server'
import type { OHLCVRow } from '@/lib/db/schema'
...
export async function GET(request: NextRequest): Promise<NextResponse<OHLCVResponse | ErrorResponse>> {
  ...
  try {
    ...
    // Check cache
    const cacheKey = getCacheKey('ohlcv', {
      symbol,
      interval,
      start: String(start),
      end: String(end),
    ...
    const cached = getCached<OHLCVRow[]>(cacheKey)
    if (cached) {
    ...
    // Cache the result
    setCache(cacheKey, rows as OHLCVRow[], queryTimeMs)

    const response = NextResponse.json({
      success: true,
      data: rows as OHLCVRow[],
      meta: {
        symbol,
        interval,
        count: rows.length,
    ...
    logger.error({ error }, 'OHLCV history query failed')
